...
// import { ChatAppRequest, ChatAppResponse } from "./models";

// export interface VectorOptions {
//     useSemanticRanker: boolean;
//     useHighAccuracyRetrieval: boolean;
//     useLightRag: boolean; // NEW toggle
// }
...
//     const body: ChatAppRequest = {
//         question,
//         history,
//         overrides: {
//             semantic_ranker: vectorOptions.useSemanticRanker,
//             semantic_captions: vectorOptions.useHighAccuracyRetrieval
//         }
//     };

//     const endpoint = vectorOptions.useLightRag ? "/ask_lightrag" : "/ask";
...
// 🔍 Perform multi-mode (Azure + LightRAG) RAG query
export async function ragSearchApi(query: string, modes: string[], idToken: string | undefined): Promise<any> {
    const headers = await getHeaders(idToken);
    const response = await fetch(`${BACKEND_URI}/rag/search`, {
        method: "POST",
        headers: { ...headers, "Content-Type": "application/json" },
...
export async function askApi(request: ChatAppRequest, idToken: string | undefined): Promise<ChatAppResponse> {
    ...
    const parsedResponse: ChatAppResponseOrError = await response.json();
    if (parsedResponse.error) throw Error(parsedResponse.error);
    return parsedResponse as ChatAppResponse;
}
